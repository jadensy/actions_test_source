...
logged_in as (
  select distinct user_id as email, [timestamp:day] as login_date
  from prod.screen_viewed
),
  ...
  (
    ...
    (
      ...
      (
        ...
        where [timestamp:day] >= [getdate():day] - 365
...
select
  ...
  count(distinct case when depth = 0 then user_id end)::float / count(distinct user_id) as viewed_no_cards
from sessions
left join logged_in
on sessions.user_id = logged_in.email
where diff = 1
  and day = login_date
group by 1
order by 1 desc