...
with dau as
(select
  [timestamp:month] as day,
  count(distinct user_id) as wau
from prod.screen_viewed
where timestamp >= '20181201'
...
referral_signups as
(select
  [statushistory_pending_createdon:month] as day,
...
plus_enrollments as
(select
  [statushistory_active_on:month] as day,
...
, core as (
  select [timestamp:month] as day, count(distinct user_id) as core_created
...
where referral_screen_viewers.day < date_trunc('month', current_date)
order by 1 desc
  )

SELECT for_unique.day,
       for_unique.unique_referrers,
       tony_month.registered_referrals,
       tony_month.core_signups,
       tony_month.plus_referrals
FROM 
for_unique INNER JOIN [tony_month] on tony_month.time_stamp=for_unique.day