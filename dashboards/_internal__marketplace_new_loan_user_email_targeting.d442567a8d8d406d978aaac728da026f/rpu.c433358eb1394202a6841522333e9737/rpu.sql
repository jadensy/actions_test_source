...
, mktp_engagement as (
  ...
  from app_engagement
  where partner not in ('LoanInATapCard', 'Personal Loan offers in 60 seconds')
  GROUP BY 1
)
...
, even_engagement as (
  ...
  from app_engagement
  where partner in ('LoanInATapCard', 'Personal Loan offers in 60 seconds')  
  GROUP BY 1
)
...
, app_uninstall as (
  SELECT 
    assignment_group,
    count(distinct case when last_uninstall_date < min_exp_timestamp then a.email else null end) as app_uninstalled_users
  FROM (
    SELECT 
      email, 
      max(timestamp) as last_uninstall_date  
    FROM iterable.mobile_app_uninstalled
    GROUP BY 1
    ) a
  JOIN mapping b ON a.email = b.email
  JOIN exp_user c ON b.userid  = c.userid
  GROUP BY 1
)
...
, final_table AS (
  SELECT 
    a.assignment_group,
    (other_mktp_rev * 1.00 / uniq_email_delivered) AS other_partner_rpu_by_email,
    COALESCE((even_rev * 1.00 / uniq_email_delivered),0) AS even_rpu_by_email,
    (other_mktp_rev * 1.00 / total_uniq_view) AS other_partner_rpu_by_mktp_tab,
    COALESCE((even_rev * 1.00 / total_uniq_view),0) AS even_rpu_by_mktp_tab
  FROM total_email_engagement a
  LEFT JOIN total_app_engagement b on a.assignment_group = b.assignment_group
  ...
  LEFT JOIN app_uninstall i ON a.assignment_group = i.assignment_group
  ORDER BY 1
)
