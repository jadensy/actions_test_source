...
, final_table AS (
  SELECT 
    ...
    uniq_email_opened,
    uniq_email_clicked,
    total_uniq_view AS mktp_tab_uniq_view,
    --        mktp_view,
    --        mktp_click,
    ...
    e.enter_even_flow,
    users_submit_api,
    (users_submit_api * 1.00 / e.enter_even_flow) AS api_submit_rate,
    ...
    (other_mktp_rev * 1.00 / mktp_tab_uniq_view) AS other_partner_rpu,
    COALESCE((even_rev * 1.00 / mktp_tab_uniq_view),0) AS even_rpu,
    ...
    ROUND((other_mktp_conv * 1.00 / NULLIF(mktp_click, 0)), 3) AS other_mktp_cvr,
