...
, email_engagement AS (
  SELECT 
    ...
    CASE 
      ...
      --WHEN engagement.campaign_id = '3873402' THEN 'control_1_leadgen_edu'
...
, final_table AS (
  SELECT 
    (case when a.assignment_group != 'control_leadgen_loan' then 'treatments' else 'control' end) as assignment_group,
    sum(uniq_email_delivered) as uniq_email_delivered,
    sum(uniq_email_opened) as uniq_email_opened,
    sum(mktp_click) + sum(even_click) as total_click,
    sum(other_mktp_conv) + sum(even_conv) as total_conv,
    sum(other_mktp_rev) + sum(even_rev) as total_rev,
    sum(mktp_click) as other_partner_click,
    sum(even_click) as even_click,
    sum(other_mktp_conv) as other_mktp_conv,
    sum(even_conv) as even_conv,
    sum(other_mktp_rev) as other_mktp_rev,
    sum(even_rev) as even_rev
--     ROUND((sum(other_mktp_conv) * 1.00 / NULLIF(sum(other_partner_click), 0)), 3) AS other_partner_cvr,
--     ROUND((sum(even_conv) * 1.00 / NULLIF(sum(even_click), 0)), 3) AS even_cvr,
--     (sum(other_mktp_rev) * 1.00 / sum(uniq_email_opened)) AS other_partner_rpu,
--     COALESCE((sum(even_rev) * 1.00 / sum(uniq_email_opened)),0) AS even_rpu
...
, final_table_agg AS (
    SELECT 
      final_table.*,
      ROUND((other_mktp_conv * 1.00 / NULLIF(other_partner_click, 0)), 3) AS other_partner_cvr,
