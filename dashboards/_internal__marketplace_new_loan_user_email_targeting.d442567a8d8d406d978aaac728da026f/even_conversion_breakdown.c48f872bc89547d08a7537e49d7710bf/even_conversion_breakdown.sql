...
, email_engagement AS (
  SELECT 
    ...
    CASE 
      WHEN engagement.campaign_id = '3894065' THEN 't1_low_income'
      WHEN engagement.campaign_id = '3905910' THEN 't2_low_bv5_score'
      WHEN engagement.campaign_id = '3905916' THEN 't3_has_liability'
      WHEN engagement.campaign_id = '3862445' THEN 'ctrl_leadgen_loan'
...
, even_conversion AS (
  SELECT 
    ...
    COUNT(case when type = 'loan' then even.userid else null end) AS even_loan_conv,
    COUNT(case when type = 'specialOffer' then even.userid else null end) AS even_specialoff_conv,
...
, final_table AS (
  SELECT 
    ...
    even_loan_conv,
    even_specialoff_conv,
    ROUND((even_loan_conv * 1.00 / NULLIF(even_conv, 0)), 3) AS even_loan_conv_pct,
    ROUND((even_specialoff_conv * 1.00 / NULLIF(even_conv, 0)), 3) AS even_specialoff_conv_pct
