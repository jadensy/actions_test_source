    ...
    from (
      select user_id, max(date_login)
        from (select user_id,
              ...
              and date("timestamp") between '2022-02-25' and '2022-03-11'
              union all
              select user_id,
                     date("timestamp") AS date_login,
                     context_os_name AS os_type
...
, exp_user as
(select user_id, assignment_group, '2022-02-25'::date as first_injection_time
from marketplace_static.exp_rec_orm_20220126 as exp
group by 1,2

union all 

select user_id, 'live_rec_treatment_group' as assignment_group, '2022-02-25'::date as first_injection_time
...
where event_timestamp >= '2022-02-25' and event_timestamp < '2022-03-11'
...
where clicked_at >= '2022-02-25' and clicked_at < '2022-03-11'
...
select 
  (case when engagement.assignment_group = '-1' then 'control (default Contentful)'
       ...
       when engagement.assignment_group = '1' then 'treatment group' 
       else engagement.assignment_group end) as assignment_group_name,
