...
, combined_count as (
  ...
  , count(distinct case when b.page_path='INSTACASH_SUBMIT_NAME' then b.user_email end) as submit_name
  , count(distinct case when c.page_path='INSTACASH_CREATE_ACCOUNT' then c.user_email end) as create_acct
  , count(distinct case when d.email is not null then d.email end) as lite
  , count(distinct case when e.page_path='INSTACASH_PII_START' then e.user_email end) as pii_start
  , count(distinct case when f.page_path='INSTACASH_DOB' then f.user_email end) as dob
  , count(distinct case when g.page_path='INSTACASH_ADDRESS' then g.user_email end) as address
  , count(distinct case when h.page_path='INSTACASH_MOBILE_PHONE_NUMBER' then h.user_email end) as mobile_phone_number
  , count(distinct case when i.page_path='INSTACASH_SSN' then i.user_email end) as ssn  
  , count(distinct case when j.page_path='INSTACASH_CONFIRM_INFORMATION' then j.user_email end) as confirm_information
...
union all
    select 12 as order_key, 'IC' as funnel_desc, ic as event_count from combined_count 
),
