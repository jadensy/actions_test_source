...
final AS (
    SELECT
        ...
        count(distinct case when (ffa_with_aff_date-lite_date)='0' then user_email end) as D0,
        count(distinct case when (ffa_with_aff_date-lite_date)>='0' and (ffa_with_aff_date-lite_date)<='7' then user_email end) as D7,
        count(distinct case when (ffa_with_aff_date-lite_date)>='0' and (ffa_with_aff_date-lite_date)<='30' then user_email end) as D30
    FROM
        install_model
    GROUP BY
        1
