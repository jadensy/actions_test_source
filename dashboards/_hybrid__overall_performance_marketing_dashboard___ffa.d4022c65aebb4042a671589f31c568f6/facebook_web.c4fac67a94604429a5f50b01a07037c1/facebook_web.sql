...
non_ios14_results as (
  ...
  left join (
    ...
    from total_cost
    where true
      and (lower(name) like '%webflow%' or lower(name) like '%homepage%')
    group by 1,2
...
select 
  ...
  "Total New SU #" * 1.0 /nullif(lp_visits.page_visits,0) as "CVR From Page Visits %",
...
left join (
  ...
  on lp_visits.week_start_date = a.week_start_date and lp_visits.ad_platform = a.ad_platform
