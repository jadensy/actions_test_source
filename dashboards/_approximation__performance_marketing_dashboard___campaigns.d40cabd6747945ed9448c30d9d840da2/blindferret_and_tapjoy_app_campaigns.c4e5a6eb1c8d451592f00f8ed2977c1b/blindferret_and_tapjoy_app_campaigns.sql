...
non_ios14_results as (
  ...
  from (
    ...
    where 1=1
      and c.ad_platform in ('blindferret','tapjoy')
  ...
  full outer join (
    select 
      [a.date_stop:week] as week_start_date,
      a.name as campaign,
      a.campaign_id,
      a.ad_platform,
    ...
    left join (select ad_platform, campaign_id from [pf_mkt_ios14_campaigns]) as b
      on b.ad_platform = a.ad_platform 
        and a.campaign_id = b.campaign_id
    where true
      and a.ad_platform in ('blindferret','tapjoy')
...
ios14_results as (
  ...
  from (
    select distinct 
      ...
      c.campaign,
      c.campaign_id,
      round(count(distinct case when [c.touchtime:week] >= '2020-03-13' and c.product = 'new core' then c.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "New Core SU",
      round(count(distinct case when c.product = 'roar money' then c.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "RM SU",
      round(count(distinct case when c.product = 'cbplus' then c.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "CB+ SU",
      round(count(distinct case when c.product = 'instacash' then c.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "IC SU",
      round(count(distinct case when c.product = 'investment standalone' then c.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "Wealth SU",
      round(count(distinct case when c.product in ('instacash','cbplus','roar money','investment standalone') then c.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "Total New SU",

      round(count(distinct case when c.product = 'roar money' then fundedrm.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "RM Funded",
      "RM Funded"*1.0/nullif("RM SU",0) as "RM Funded %",
      round(count(distinct case when c.product = 'cbplus' then cbloan.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "CB+ Funded",
      "CB+ Funded"*1.0/nullif("CB+ SU",0) as "CB+ Funded %",
      round(count(distinct case when c.product = 'instacash' then takeic.email end)* 1.0 / avg(att_results.opt_in_rate),0) as "IC Taken",
  ...
  full outer join (
    select
      ...
      sum(a.total_impressions) as total_impressions
    from total_cost as a
    join (select ad_platform, campaign_id from [pf_mkt_ios14_campaigns]) as b
      on b.ad_platform = a.ad_platform 
        and a.campaign_id = b.campaign_id
    where true
      and a.ad_platform in ('blindferret','tapjoy')
