...
landing_page_agg as (
  select 
    case when page_path like '/google%' then 'google'
         ...
         when page_path like '/snapchat%' then 'snapchat'
  ...
  where true 
    ...
    and (unbounce.page_path like '/bing%' or unbounce.page_path like '/google%' or unbounce.page_path like '/facebook%' or unbounce.page_path like '/snapchat%')
...
select distinct
  coalesce(c.week_start_date, cost.week_start_date, lp_visits.week_start_date) as week_start_date,
  coalesce(c.ad_platform, cost.ad_platform, lp_visits.ad_platform) as ad_platform,
  coalesce(c.campaign, cost.campaign) as campaign,
  coalesce(c.campaign_id, cost.campaign_id, lp_visits.ad_platform) as campaign_id,
