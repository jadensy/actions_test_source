...
select distinct
  coalesce(c.reporting_day, cost.reporting_day) as reporting_day,
...
from (
  select distinct 
    ...
    date(c.touchtime) as reporting_day,
    c.ad_platform,
    c.campaign,
    c.campaign_id,
...
full outer join (
  ...
  from total_cost
  where true
--     and ad_platform = 'google'
    and (lower(campaign) like '%webflow%' or lower(campaign) like '%homepage%')
  group by 1,2,3,4
  having sum(total_cost) > 0
) cost on c.reporting_day = cost.reporting_day and c.ad_platform = cost.ad_platform and c.campaign_id = cost.campaign_id

left join (
  select 
    date("timestamp") as reporting_day,
    ad_platform,
  ...
  on lp_visits.reporting_day = coalesce(c.reporting_day, cost.reporting_day) 
