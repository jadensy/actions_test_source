...
final AS (
    SELECT
        ...
        count(
            CASE
                WHEN lite_register IN ('no_lite', 'mobileApp')
                AND ad_platform NOT IN ('blindferret', 'tapjoy') THEN appsflyer_id
            END
        ) normal_install,
        count(
            CASE
                WHEN lite_register IN ('no_lite', 'mobileApp')
                AND ad_platform IN ('blindferret', 'tapjoy') THEN appsflyer_id
            END
...
SELECT
    ...
    (1.00 * app_install_campaign / install) install_campaign_pct,
    (1.00 * pre_signup / install) pre_signup_pct
FROM
    final
ORDER BY
    1 DESC