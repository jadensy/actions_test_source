...
, aggre_rm_paychecks as(
  select
    account_id
    , bv_transaction_id
    , ml_user_id
    , transaction_date
    , lag(transaction_date, 1) over (partition by ml_user_id order by transaction_date) as prev_paycheck_ts
    , lag(bv_transaction_id, 1) over (partition by ml_user_id order by transaction_date) as prev_paycheck_id
    , abs(datediff(day, [transaction_date:day], [prev_paycheck_ts:day])) as days_since_prev_txn
  from rm_paychecks_raw
  order by ml_user_id, transaction_date
)

, churning_paychecks_a as(
  select
    ml_user_id
    , prev_paycheck_id as churning_paycheck
    , prev_paycheck_ts as churning_ts
  from aggre_rm_paychecks
  where days_since_prev_txn > 35
)

, churning_paychecks_b as(
  select
    distinct(ml_user_id)
    , bv_transaction_id as churning_paycheck
    , transaction_date as churning_ts
  from(
    select
      ml_user_id
      , bv_transaction_id
      , transaction_date
      , row_number() over (partition by ml_user_id order by transaction_date desc) rn
    from rm_paychecks_raw
  )
  where rn = 1
  and date(transaction_date) < date(getdate()) - interval '35 days'
)

, join_churning_paychecks as(
  select * from(
    select * from churning_paychecks_a
    union
    select * from churning_paychecks_b
  )
  order by ml_user_id, churning_ts desc
)
...
, aggre_2 as(
  select
    a.*
    , b.paycheck_status
  from join_churning_paychecks a
  left join aggre b
    on a.ml_user_id = b.ml_user_id
...
select
  [churning_ts:month] as churning_month
  , paycheck_status as paycheck_type
  , count(distinct ml_user_id) as total_paycheck_churned_users
from aggre_2
group by 1,2
order by 1 desc