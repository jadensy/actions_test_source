...
with pc_threshold as(
  select 45 as threshold
)
...
, aggre_rm_paychecks as(
  select
    ...
    , bv_transaction_id
    , transaction_date
    , lag(transaction_date, 1) over (partition by ml_user_id order by transaction_date) as prev_paycheck_date
    , abs(datediff(day, [transaction_date:day], [prev_paycheck_date:day])) as days_since_prev_paycheck
  from rm_paychecks_raw
  order by 1,3
)

, reactivating_paycheck as(
  select
    *
  from aggre_rm_paychecks
  where days_since_prev_paycheck > (select threshold from pc_threshold)
)

, users_w_reactivation as (
  select 
    distinct(ml_user_id)
  from reactivating_paycheck
)

select
  count(case when ml_user_id in (select ml_user_id from users_w_reactivation) then ml_user_id end)::float /  count(distinct ml_user_id) as pct_reactivation
from aggre